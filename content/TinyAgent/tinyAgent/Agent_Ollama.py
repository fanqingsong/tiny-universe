...
from tinyAgent.LLM_Ollama import InternLM2Chat
from tinyAgent.tool_Ollama import Tools
...
class Agent_Ollama:
    ...
    def parse_latest_plugin_call(self, text):
        print("--------------- parse_latest_plugin_call ------------")
        print(text)
        print("--------------- parse_latest_plugin_call ------------")
    ...
    def call_plugin(self, plugin_name, plugin_args):
        ...
        if plugin_name == 'bing_search':
            return '\nObservation:' + self.tool.bing_search(**plugin_args)

    def text_completion(self, text, history=[]):
        text = "\nQuestion:" + text
        response = self.model.chat(text, history, self.system_prompt)
        print("========== first chat response ==========")
        print(response)

        plugin_name, plugin_args, response = self.parse_latest_plugin_call(response)

        print("========== after call parse latest plugin ==========")
        print(f'plugin_name={plugin_name}')
        print(f'plugin_args={plugin_args}')
        print(f'response={response}')

        if plugin_name:
            response += self.call_plugin(plugin_name, plugin_args)

        print("========== after call plugin ==========")
        print(response)
